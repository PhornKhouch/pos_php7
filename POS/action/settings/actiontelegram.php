<?php
    ...
    if(isset($_POST['btnsave'])){
        ...
        if(!empty($_POST['id'])) {
            $id = $_POST['id'];
            $sql = "UPDATE `telegram` SET `token`='$token', `chatid`='$chatid', `groupid`='$groupid', `status`='$status' WHERE `id`='$id'";
        } else {
            $sql = "INSERT INTO `telegram`(`token`, `chatid`, `groupid`, `status`) VALUES ('$token','$chatid','$groupid','$status')";
        }
